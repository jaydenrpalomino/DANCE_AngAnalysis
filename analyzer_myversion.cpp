...
int Create_Analyzer_Histograms(Input_Parameters input_params) {
  ...
  int NoOfEnergyBins=GammaE_NoOfBins; // defined in global.h
  ...
  if(input_params.Analysis_Stage==1 || input_params.Read_Simulation==1) {
    ...
    En_Esum_Mcl=new TH3F("En_Etot_Mcl","En_Etot_Mcl",NEbins,x,NoOfEnergyBins,EtotBins,20,Mbins); //x=5000
    ...
    #endif
    cout << "The value of nqgates is: " << input_params.NQGates << endl;
    cout << "The value of QGatedSpectra is: " << input_params.QGatedSpectra << endl;
...
int Analyze_Data(std::vector<DEVT_BANK> eventvector, Input_Parameters input_params, Analysis_Parameters *analysis_params) {
  ...
  //DANCE Events
  //cout << "check validity " << devent.Valid << endl;//jrp //most are 1
  if(devent.Valid == 1) {
    ...
    if(input_params.Analysis_Stage==1 || input_params.Read_Simulation ==1 ) {
      //cout << "devent.timestamp[0]= " << devent.timestamp[0] << " last valid timestamp= " << last_valid_devent_timestamp << " last timestamp= " << last_devent_timestamp << " check analysis_params->last_timestamp " << analysis_params->last_timestamp[T0_ID] << endl;
      //cout << "check analysis_params->last_timestamp " << analysis_params->last_timestamp[T0_ID] << endl;

      // jrp-- my bad runs never get in this loop:
      if(analysis_params->last_timestamp[T0_ID] > 0) {
        //cout << "check last_timestamp " << analysis_params->last_timestamp[T0_ID] << endl; //checks timestamps that are not zero
        //cout << "devent.timestamp[0]= " << devent.timestamp[0] << " last valid timestamp= " << last_valid_devent_timestamp << " last timestamp= " << last_devent_timestamp << " check analysis_params->last_timestamp " << analysis_params->last_timestamp[T0_ID] << endl;
      ...
      //cout << "check validity " << devent.Valid << endl;//jrp //all validity = 0
      //Handle various events and do some physics
      if(devent.Valid==1) {
        //cout << "inside the valid event loop " << devent.Valid << endl;
        ...
        /// never gets in this loop:
        //Crystal mult 1
        //cout << "check mult " << devent.Crystal_mult << endl;
        if(devent.Crystal_mult == 1) {
          //cout << "in the crystal mult == 1 loop " << endl;
