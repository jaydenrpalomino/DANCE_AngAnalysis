...
TH2D *hCrystalIDvsTOF_fast;           //TOF for each crystal, 10 binning for first 10 us
...
int Create_Analyzer_Histograms(Input_Parameters input_params) {
  ...
  //RAW TOF
  hCrystalIDvsTOF = new TH2D("ID_CrystalTOF","ID_CrystalTOF",20100,-10000,2000000,162,0,162); //TOF for each crystal 
  // ajc TOF per crystal, first 10 us.
  hCrystalIDvsTOF_fast = new TH2D("ID_CrystalTOF_fast","ID_CrystalTOF_fast",1100,-1000,10000,162,0,162); 
...
int Analyze_Data(std::vector<DEVT_BANK> eventvector, Input_Parameters input_params, Analysis_Parameters *analysis_params) {
  ...
  if(devent.Valid == 1) {
	...
	for(int kay=0; kay<devent.Crystal_mult; kay++) {
	  ...
	  if (devent.tof[kay] < 1.1e4 ) //only fill if it is in the histo range
	    hCrystalIDvsTOF_fast->Fill(devent.tof[kay],devent.Crystal_ID[kay],1);
  ...
  if (input_params.EnResGated_Ang_Spectra) {
    ...
    if (devent.Valid == 1 && analysis_params->last_timestamp[T0_ID] > 0 && devent.Crystal_mult >= 1 && devent.Crystal_mult < maxMultiplicity && devent.Cluster_mult == devent.Crystal_mult) {
	  ...
	  //cout << "detector" << det << "  angle " << ng_angle << endl;
          for (int kay = 0; kay < input_params.NEnResGates_Ang; ++kay) {
